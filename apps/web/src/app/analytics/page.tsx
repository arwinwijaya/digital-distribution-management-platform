...
type AIData = {
  ...
  forecast: { predictions: Array<{ period: string; forecast_sales: string; forecast_orders: number }>; confidence: string; data_sufficiency: { sufficient: boolean; level: string }; method: string };
...
export default function AnalyticsPage() {
  ...
  const load = useCallback(async (authToken: string) => {
    ...
    try {
      ...
      setData({ recommendations: bodies[0].data.recommendations, forecast: bodies[1].data, segmentation: bodies[2].data });
    } catch (loadError) {
      setData(null);
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-8">
      ...
      {data && <div className="grid gap-6 lg:grid-cols-3">
        <section className="rounded border bg-white p-5 lg:col-span-2"><h2 className="mb-3 text-xl font-semibold">Recommendations</h2>{data.recommendations.length === 0 ? <p className="text-gray-500">No purchase history is available yet.</p> : <ul className="space-y-2">{data.recommendations.map((item) => <li key={item.product_id} className="flex justify-between border-b py-2"><span>{item.name}<small className="ml-2 text-gray-500">{item.reason}</small></span><span>Rp {Number(item.price).toLocaleString('id-ID')}</span></li>)}</ul>}</section>
        ...
        <section className="rounded border bg-white p-5 lg:col-span-3"><h2 className="mb-3 text-xl font-semibold">Four-week forecast</h2><p className="mb-3 text-sm text-gray-500">Method: {data.forecast.method}. Confidence: {data.forecast.confidence} ({data.forecast.data_sufficiency.level} data).</p>{data.forecast.predictions.length === 0 ? <p className="text-gray-500">Not enough positive history to make a forecast.</p> : <div className="grid gap-3 sm:grid-cols-4">{data.forecast.predictions.map((point) => <article key={point.period} className="rounded bg-gray-50 p-3"><time className="text-sm text-gray-500">{point.period}</time><p className="font-semibold">Rp {Number(point.forecast_sales).toLocaleString('id-ID')}</p><p className="text-sm text-gray-600">{point.forecast_orders} orders</p></article>)}</div>}</section>
      </div>}
    </main>
  );
}
