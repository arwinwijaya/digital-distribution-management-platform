...
function useInvoiceData(): InvoiceState {
  ...
  const load = useCallback(async (nextToken: string, page = 1) => {
    setLoading(true);
    setError(null);
    try {
  ...
  return { token, invoices, meta, loading, error, setToken, load };
}

function InvoiceSummary({ invoices, total }: { invoices: Invoice[]; total: number }) {
  const totalBalance = invoices.reduce((sum, invoice) => sum + Number(invoice.balance_amount || 0), 0);
  return <section className="mb-6 grid gap-4 sm:grid-cols-2">
    <StatCard label="Invoice" value={total.toLocaleString('id-ID')} icon="🧾" />
    <StatCard label="Sisa pada halaman ini" value={money(totalBalance)} icon="💰" />
  </section>;
}
...
function InvoiceLogin({ onLogin }: { onLogin: (token: string) => void }) {
  return <div className="mx-auto max-w-6xl">
    <PageHeader title="Invoice" description="Pantau invoice dan saldo piutang." />
    <LoginForm expectedRole={['finance', 'admin', 'outlet']} onLogin={(nextToken) => onLogin(nextToken)} />
  </div>;
}
...
export default function InvoicesPage() {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <InvoiceSummary invoices={data.invoices} total={data.meta.total} />
    ...
    <InvoiceHistory invoices={data.invoices} meta={data.meta} loading={data.loading} token={data.token} load={data.load} />
  </div>;
}
