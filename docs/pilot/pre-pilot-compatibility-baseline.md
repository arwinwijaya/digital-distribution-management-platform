...
- **Resolved — OrderTest re-approval expectation:** `OrderTest` previously asserted `422` on re-approval, but `OrderController::approveInTransaction` returns `200` with invoice reuse. The implementation behavior (200) is the compatibility baseline; the stale `OrderTest` assertions have been corrected to assert idempotent `200` plus exactly one invoice and one `Confirmed` history row (see `test_reapproving_already_confirmed_order_is_idempotent` and `test_concurrent_admin_approvals_append_one_confirmed_history`), matching `InvoiceTest::test_approval_retry_reuses_invoice`.
...
- **Note — SQLite HTTP race determinism:** the `tests/Feature/Concurrency` SQLite HTTP race harnesses (`OrderConcurrencyTest`, `DeliveryConcurrencyTest`, `PrePilotConcurrencyCompatibilityTest`) release two real workers through a barrier file and assert the retry/read-back result, not simultaneous row locks. A historical intermittent failure of `OrderConcurrencyTest::test_concurrent_admin_approvals_append_one_confirmed_history` (`[200, 500]` instead of `[200, 200]`) was investigated and did not reproduce across 14 consecutive suite runs on the current harness; it was confirmed pre-existing on the old `OrderTest` copy, not a refactor regression. If it recurs, treat it as a harness timing issue (barrier/wait), not a production-contract failure.
