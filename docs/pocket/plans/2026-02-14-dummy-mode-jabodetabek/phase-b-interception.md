...
**Sequencing:** These four tasks share no files and can run concurrently. Do not treat the intra-phase `[prereq]` markers as "must run first within this phase" — they mean "no dependency inside this file"; the real prerequisite is Phase A.
...
Story 2 — Read paths (inline-fetch pages)
  ...
  Modify: apps/web/src/components/OrderForm.tsx (READ path in B-T3; POST is B-T4)
...
1. Write failing test for: DI reads return dummy aggregates with zero network while ON
   ...
   Test intent:
   Given the dummy store ON (populated with T6 aggregates via `setDummyGenerator`) and a jest.fn() global fetch
...
5. Write failing test for: operations reads (incl. issue detail) never hit backend while ON
   ...
   Test intent:
   Given the store ON with dummy issues present
...
5. Write failing test for: sales performance + admin users guards return typed dummies offline
   Test file: `apps/web/src/app/admin/outlets/dummy-guard.test.ts` (append)
   Level: unit

   Test intent:
   Given the store ON
   When `fetchMyPerformance('token')` and `fetchAdminUsers('token', {})` are awaited
   Then:
   - `fetchMyPerformance` resolves to a `MyPerformance`-shaped object with non-zero period totals
   - `fetchAdminUsers` resolves to `UsersListResult` with ≥1 user
   - global fetch was NEVER called in either case

   Exercise through:
   - exported functions of `sales/performance/api.ts` and `admin/users/api.ts`
...
## DELIVERABLE
Given store ON, When any guarded admin/sales read is awaited, Then correctly-shaped dummy returned AND zero fetch
...
Must-have:
  ...
  - `OrderForm.tsx` product-catalog read is guarded (write stays B-T4)
  - Tests written BEFORE implementation (TDD — not after)
  - Commit message follows conventional commits format
...
5. Write failing test for: write guards stop network and mutations stay ephemeral
   ...
   Test intent:
   Given store ON with a jest.fn() global fetch
   When `createSalesOrder(token, payload)` and `updateDummyDelivery(...)` are invoked via their call-site modules
   Then:
   - the order/delivery reflect locally AND global fetch was NEVER called
   ...
   Exercise through:
   - exported `createSalesOrder` from `apps/web/src/app/sales/orders/api.ts`; `updateDummyDelivery` + the delivery page PATCH call-site; store `toggle()`
...
## DELIVERABLE
Given store ON at /orders, When submitting an order, Then it appears with a `dummy-` id AND linked dummy payment/invoice/delivery exist AND zero fetch
...
Scope note: `admin/orders#approve` (PUT `/orders/:id/approve`) is NOT guarded — the Admin order-approval menu is excluded from spec "Cakupan menu penuh"; leave it hitting the backend (or confirm exclusion with the user in Phase C).
...
Rollback note:
  - Delete `mutations.ts` + test; revert the eight call-site edits. Pages fall back to real writes.
