...
import { Button, Card, EmptyState, PageHeader, StatusBadge, Table, TablePagination, TableSummary } from '@/components/ui';
...
export default function AdminOrdersPage() {
  ...
  return <div className="mx-auto max-w-6xl"><PageHeader title="Kelola pesanan" description="Tinjau, setujui, dan pantau pesanan outlet." />{error && <p role="alert" className="mb-5 rounded-lg border border-danger-200 bg-danger-50 p-3 text-sm text-danger-700">{error}</p>}<div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]"><Card className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-3"><TableSummary total={total ?? orders.length} noun="pesanan" /></div>{loading && orders.length === 0 ? <p className="p-8 text-sm text-gray-500">Memuat pesanan...</p> : <Table columns={columns} rows={orders} rowKey={(order) => order.id} sortableColumns={['created_at', 'updated_at', 'order_id', 'status', 'total_amount']} sort={sort} onSort={(column) => { const next = toggleSort(sortRef.current, column); setSort(next); if (token) void loadOrders(token, { resetCursor: true, sort: next }); }} empty={<EmptyState icon={<span>🛒</span>} title="Belum ada pesanan" description="Pesanan baru dari outlet akan muncul di sini." />} />}<TablePagination cursor={cursor} limit={PAGE_LIMIT} total={total} hasMore={hasMore} onPageChange={(nextCursor) => { setCursor(nextCursor); if (token) void loadOrders(token, { cursor: nextCursor }); }} /></Card><Card className="p-5">{selected ? <><div className="flex items-center justify-between"><h2 className="text-base font-semibold text-gray-900">{selected.order_id}</h2><StatusBadge status={selected.status} /></div><p className="mt-3 text-lg font-bold text-gray-900">Rp {Number(selected.total_amount).toLocaleString('id-ID')}</p><h3 className="mt-6 border-b border-gray-100 pb-2 text-sm font-semibold text-gray-900">Riwayat status</h3><ol className="mt-3 space-y-3 border-l-2 border-primary-100 pl-4 text-sm text-gray-600">{(selected.status_history || []).map((history) => <li key={`${history.status}-${history.created_at}`}><span className="font-medium text-gray-800">{history.status}</span><br /><span className="text-xs">{new Date(history.created_at).toLocaleString('id-ID')}</span></li>)}</ol></> : <EmptyState icon={<span>👆</span>} title="Pilih pesanan" description="Pilih nomor pesanan di tabel untuk melihat detail." />}</Card></div></div>;
}
